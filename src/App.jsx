...
export default function App() {
  ...
  const handleLogin = (u) => { setUser(u); setPagina('dashboard') }
  const handleLogout = () => { setUser(null); setPagina('dashboard') }
  const podeAdmin = user.perfil === 'planejamento'
  const podeForm = user.perfil === 'secretario' || user.perfil === 'planejamento'
  const renderPagina = () => {
    if (pagina === 'form' && podeForm) return <SecretariaForm user={user} />
    if (pagina === 'admin' && podeAdmin) return <AdminPanel user={user} />
    return <Dashboard user={user} onNavegar={setPagina} />
  }
  return (
      ...
      <nav style={{ background: '#111827', borderBottom: '1px solid #1e2d45', padding: '0 24px', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '56px' }}>
          ...
          <div style={{ display: 'flex', gap: '4px' }}>
            ...
            {podeForm && <button onClick={() => setPagina('form')} style={{ padding: '6px 14px', borderRadius: '7px', border: 'none', fontSize: '13px', cursor: 'pointer', fontFamily: 'Sora, sans-serif', background: pagina === 'form' ? '#00d4aa22' : 'transparent', color: pagina === 'form' ? '#00d4aa' : '#64748b' }}>📋 Atualizar Ações</button>}
            {podeAdmin && <button onClick={() => setPagina('admin')} style={{ padding: '6px 14px', borderRadius: '7px', border: 'none', fontSize: '13px', cursor: 'pointer', fontFamily: 'Sora, sans-serif', background: pagina === 'admin' ? '#00d4aa22' : 'transparent', color: pagina === 'admin' ? '#00d4aa' : '#64748b' }}>⚙️ Admin</button>}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <button onClick={handleLogout} style={{ padding: '6px 14px', borderRadius: '7px', border: '1px solid #1e2d45', fontSize: '12px', cursor: 'pointer', fontFamily: 'Sora, sans-serif', background: 'transparent', color: '#64748b' }}>Sair</button>
          </div>
        </div>
      </nav>
